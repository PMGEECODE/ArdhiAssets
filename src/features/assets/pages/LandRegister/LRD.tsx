...
import { useLandRegisterStore } from "../../store/landRegisterStore";
import type { LandRegister } from "../../store/landRegisterStore";
import Button from "../../components/ui/Button";
import Card from "../../components/ui/Card";
import LandParcelMap from "../../components/LandParcelMap";

import { KshIcon } from "../../components/ui/icons/KshIcon";
...
const LandRegisterDetail: React.FC = () => {
  ...
  const statistics = useMemo(() => {
    if (!selectedLandRegister) return null;

    // Generate sparkline data (mock trend data for visualization)
    ...
    const sizeHa = Number(selectedLandRegister.size_ha) || 0;
    const fairValue = Number(selectedLandRegister.fair_value) || 0;
    const acquisitionAmount =
      Number(selectedLandRegister.acquisition_amount) || 0;
    const rentalIncome = Number(selectedLandRegister.annual_rental_income) || 0;

    // Calculate value appreciation (mock trend)
    const valueAppreciation =
      fairValue > 0 && acquisitionAmount > 0
        ? ((fairValue - acquisitionAmount) / acquisitionAmount) * 100
        : 0;

    // Calculate rental yield
    const rentalYield =
      fairValue > 0 && rentalIncome > 0 ? (rentalIncome / fairValue) * 100 : 0;

    return {
      size: {
        value: sizeHa,
        unit: "ha",
        trend: 0, // Land size doesn't change
        subtitle: selectedLandRegister.category || "Land",
        sparkline: generateSparklineData(sizeHa, sizeHa * 0.05),
      },
      fairValue: {
        value: fairValue,
        ...
        subtitle:
          acquisitionAmount > 0
            ? `Acquired at Ksh ${acquisitionAmount.toLocaleString()}`
            : "Current market value",
        sparkline: generateSparklineData(fairValue, fairValue * 0.1),
      },
      rentalIncome: {
        value: rentalIncome,
        trend: rentalYield,
        subtitle:
          rentalYield > 0
            ? `${rentalYield.toFixed(2)}% yield`
            : "Annual rental income",
        sparkline: generateSparklineData(rentalIncome, rentalIncome * 0.15),
      },
      status: {
        disputed: selectedLandRegister.disputed === "Disputed",
        surveyed: selectedLandRegister.surveyed === "Surveyed",
        planned: selectedLandRegister.planned_unplanned === "Planned",
        hasEncumbrances: !!selectedLandRegister.encumbrances,
      },
    };
  }, [selectedLandRegister]);
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div className="mt-16">
                  <div className="flex items-baseline justify-between mb-1">
                    <h3 className="text-3xl font-bold">
                      {statistics.size.value.toFixed(2)}
                    </h3>
                    <span className="text-sm font-medium text-emerald-100">
                      {statistics.size.unit}
                    </span>
                  </div>
                  <p className="text-sm text-emerald-100 mb-3">Land Size</p>
                  <p className="text-xs text-emerald-200 mb-2">
                    {statistics.size.subtitle}
                  </p>
                  <MiniSparkline
                    data={statistics.size.sparkline}
                    color="bg-white/30"
                  />
                </div>
              </div>
                ...
                <div className="mt-16">
                  <div className="flex items-baseline justify-between mb-1">
                    <h3 className="text-2xl font-bold">
                      {statistics.fairValue.value > 0
                        ? `${(statistics.fairValue.value / 1000000).toFixed(
                            1
                          )}M`
                        : "N/A"}
                    </h3>
                    {statistics.fairValue.trend !== 0 && (
                      <div
                      ...
                      >
                        ...
                        {Math.abs(statistics.fairValue.trend).toFixed(1)}%
                      </div>
                    )}
                  </div>
                  <p className="text-sm text-blue-100 mb-3">Fair Value</p>
                  <p className="text-xs text-blue-200 mb-2 truncate">
                    {statistics.fairValue.subtitle}
                  </p>
                  <MiniSparkline
                    data={statistics.fairValue.sparkline}
                    color="bg-white/30"
                  />
                </div>
              </div>
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-purple-500 to-purple-700 rounded-lg shadow-lg transition-transform hover:scale-105">
              <div className="p-6 text-white">
                ...
                <div className="mt-16">
                  <div className="flex items-baseline justify-between mb-1">
                    <h3 className="text-2xl font-bold">
                      {statistics.rentalIncome.value > 0
                        ? `${(statistics.rentalIncome.value / 1000).toFixed(
                            0
                          )}K`
                        : "N/A"}
                    </h3>
                    {statistics.rentalIncome.trend > 0 && (
                      <div className="flex items-center text-sm font-medium text-green-200">
                        <TrendingUp className="w-4 h-4 mr-1" />
                        {statistics.rentalIncome.trend.toFixed(1)}%
                      </div>
                    )}
                  </div>
                  <p className="text-sm text-purple-100 mb-3">Annual Rental</p>
                  <p className="text-xs text-purple-200 mb-2 truncate">
                    {statistics.rentalIncome.subtitle}
                  </p>
                  <MiniSparkline
                    data={statistics.rentalIncome.sparkline}
                    color="bg-white/30"
                  />
                </div>
              </div>
            ...
            <div
              className={`relative overflow-hidden rounded-lg shadow-lg transition-transform hover:scale-105 ${
                statistics.status.disputed || statistics.status.hasEncumbrances
            ...
            >
              <div className="p-6 text-white">
                <div className="absolute top-4 left-4 p-2 bg-white/20 rounded-lg backdrop-blur-sm">
                  {statistics.status.disputed ||
                  statistics.status.hasEncumbrances ? (
                ...
                <div className="mt-16">
                  <div className="mb-3">
                    ...
                    <p className="text-sm opacity-90">Land Status</p>
                  </div>
                  <div className="space-y-1 text-xs opacity-90">
                    <div className="flex items-center justify-between">
                      <span>Disputed:</span>
                      <span className="font-medium">
                        {statistics.status.disputed ? "Yes" : "No"}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span>Surveyed:</span>
                      <span className="font-medium">
                        {statistics.status.surveyed ? "Yes" : "No"}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span>Planned:</span>
                      <span className="font-medium">
                        {statistics.status.planned ? "Yes" : "No"}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span>Encumbrances:</span>
                      <span className="font-medium">
                        {statistics.status.hasEncumbrances ? "Yes" : "No"}
                      </span>
                    </div>
                  </div>
                </div>

...
import {
  ...
  type PortableItem,
} from "../../../../shared/store/portableItemsStore";
...
const PortableItemsForm: React.FC = () => {
  ...
  const [formData, setFormData] = useState<Partial<PortableItem>>({
    asset_description: "",
    financed_by: "",
    serial_number: "",
    tag_number: "",
  ...
  // Auto-calculate financial fields when purchase amount or depreciation rate changes
  useEffect(() => {
    if (
      autoCalculate &&
  ...
  const calculateFinancialFields = () => {
    ...
    // Determine the "end" of the depreciation period
    ...
    // Calculate daily depreciation and accumulated value
  ...
  const loadPortableItem = async () => {
    ...
    try {
      ...
      if (item) {
        setFormData(item);
        // Disable auto-calculate for existing items to preserve their values
        setAutoCalculate(false);
      }
    } catch (error) {
      toast.error("Failed to load portable item");
  ...
  const validateTagNumber = async (tagNumber: string) => {
    ...
    try {
      const response = await axios.post(
        `${
          import.meta.env.VITE_API_URL || "http://localhost:3001"
        }/portable-items/validate-tag${isEditing ? `?asset_id=${id}` : ""}`,
  ...
  const handleInputChange = (field: keyof PortableItem, value: any) => {
    ...
    // If user manually changes calculated fields, disable auto-calculate
    const prevFormData = formData; // Declare prevFormData to use in the condition
    if (
      [
        "annual_depreciation",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && id) {
        await updatePortableItem(id, formData);
        toast.success("Portable item updated successfully");
      } else {
        await createPortableItem(
          formData as Omit<PortableItem, "id" | "created_at" | "updated_at">
        );
        toast.success("Portable item created successfully");
      }
      ...
      toast.error(
        isEditing
          ? "Failed to update portable item"
          : "Failed to create portable item"
      );
    }
  };
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">
            {isEditing ? "Edit Portable Item" : "Add Portable Item"}
          </h1>
          <p className="mt-2 text-gray-600">
            {isEditing
              ? "Update portable item information"
              : "Create a new portable item entry"}
          </p>
        </div>
      </div>
            ...
            {autoCalculate && formData.delivery_installation_date && (
              <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                <p className="text-sm text-amber-800">
                  Accumulated depreciation is calculated based on years since
...
export default PortableItemsForm;

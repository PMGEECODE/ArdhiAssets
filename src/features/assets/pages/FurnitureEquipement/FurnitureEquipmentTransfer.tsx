...
interface FurnitureEquipmentTransfer {
  id: string;
  asset_id: string;
...
const FurnitureEquipmentTransferPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { selectedFurnitureEquipment, getFurnitureEquipmentById } =
    useFurnitureEquipmentStore();

  const [asset, setAsset] = useState<FurnitureEquipment | null>(null);
  const [transferHistory, setTransferHistory] = useState<
    FurnitureEquipmentTransfer[]
  >([]);
  ...
  useEffect(() => {
    const loadAsset = async () => {
      if (id) {
        const fetchedAsset = await getFurnitureEquipmentById(id);
        if (fetchedAsset) {
  ...
  const fetchTransferHistory = async (assetId: string) => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}/furniture-equipment/${assetId}/transfers/`
      );
  ...
  const handleTransfer = async () => {
    ...
    try {
      const response = await axios.post(
        `${API_URL}/furniture-equipment/${asset.id}/transfer`,
        payload
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <Card className="bg-white border border-gray-200 shadow-sm">
            <div className="p-6">
              <h2 className="pb-2 mb-4 text-lg font-semibold text-gray-900 border-b border-gray-100">
                Current Furniture Equipment Information
              </h2>
              ...
              {isLoadingTransfers ? (
                ...
                <div className="py-8 text-center">
                  ...
                  <div className="text-sm text-primary-400">
                    This furniture equipment has not been transferred yet.
                  </div>
                </div>
              ) : (
      ...
      {showSuccessModal && (
        <SuccessModal
          message="The furniture equipment transfer was completed successfully."
...
export default FurnitureEquipmentTransferPage;

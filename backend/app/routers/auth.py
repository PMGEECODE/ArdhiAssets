...
async def refresh_token_endpoint(
    ...
    - Session-level validation (optional if header missing)
    ...
    # If no session ID provided, we validate user is active and proceed
    # This prevents premature logout when the header is missing
    session_id = request.headers.get("X-Session-Id")
    session = None

    if session_id:
        print(f"[v0] X-Session-Id provided: {session_id}")
        ...
        if not session or not session.is_active or session.expires_at <= now_utc:
            print(f"[v0] Session validation failed")
            await audit_service.create_audit_log(
                db=db,
                user=None,
        ...
        print("[v0] No X-Session-Id header provided - performing fallback validation")
        # Fallback: Get most recent active session for this user
        stmt = select(UserSession).where(
            and_(
        ...
        if not session:
            print("[v0] No active session found for user in fallback")
            await audit_service.create_audit_log(
                db=db,
                user=None,
                action="TOKEN_REFRESH_FAILED_NO_SESSION",
                entity_type="auth",
                details=f"No active session found for user_id: {user_id}",
                ip_address=client_ip
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        print(f"[v0] Using fallback session: {session.id}")
    ...
    await audit_service.create_audit_log(
        ...
        entity_type="auth",
        details=f"Access token refreshed for session {session.id}",
        ip_address=client_ip
    )
...
async def logout(
    ...
    session_id = request.headers.get("X-Session-Id")
    if session_id:
        success = await session_service.delete_session_and_tokens(db, session_id)
        print(f"[v0] Deleted session {session_id} for user {current_user.id}: {success}")
    else:
...
async def terminate_session(
    ...
    """
    Terminate a session and delete associated refresh tokens

    Admin can terminate any session. Users can only terminate their own.
    This endpoint now completely removes the session and refresh tokens.
    """
    session = await session_service.get_session_by_id(db, session_id)
    if not session:
        raise HTTPException(
    ...
    await audit_service.create_audit_log(
        ...
        entity_type="session",
        details=f"Session {session_id} for user {session.user_id} terminated and deleted",
        ip_address=audit_service.get_client_ip(request)
    )

    await redis_pubsub.publish_session_terminated(str(session.user_id), session_id)

    return {
        "message": f"Session {session_id} terminated and deleted successfully",

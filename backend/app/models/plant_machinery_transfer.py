...
class PlantMachineryTransfer(Base):
    """
    SQLAlchemy model representing transfer history for Plant Machinery.
    Tracks asset re-assignments, location changes, and transfer metadata.
    """

    __tablename__ = "plant_machinery_transfers"

    # =====================
    # Table Indexes
    # =====================
    __table_args__ = (
        Index("idx_plant_transfer_asset_date", "plant_machinery_id", "transfer_date"),
        Index("idx_plant_transfer_location", "transfer_location"),
        Index("idx_plant_transfer_assigned_to", "assigned_to"),
    )
    ...
    # Asset Relationship
    # =====================
    plant_machinery_id = Column(
        UUID(as_uuid=True),
        ForeignKey("plant_machinery.id"),
        nullable=False,
        index=True,
    )
    ...
    # Relationships
    # =====================
    plant_machinery = relationship("PlantMachinery", backref="transfers")

    def __repr__(self):
        return (
            f"<PlantMachineryTransfer(id={self.id}, plant_machinery_id={self.plant_machinery_id}, "
